...
async def test_mark_alert_sent_sets_flag(volume_db):
    await increment_daily_search_count(str(volume_db))
    updated = await mark_alert_sent(str(volume_db))
...
async def test_increment_daily_search_count_returns_none_on_db_error(volume_db):
    with patch(
        "src.sohnbot.persistence.search_volume.aiosqlite.connect",
        side_effect=aiosqlite.Error("db offline"),
    ):
        result = await increment_daily_search_count(str(volume_db))
...
async def test_claim_alert_slot_claims_once_when_threshold_exceeded(volume_db):
    ...
    first_claim = await claim_alert_slot(str(volume_db), threshold=3)
    second_claim = await claim_alert_slot(str(volume_db), threshold=3)
...
async def test_claim_alert_slot_returns_none_when_threshold_not_exceeded(volume_db):
    ...
    claim = await claim_alert_slot(str(volume_db), threshold=3)
