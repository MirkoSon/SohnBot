...
from src.sohnbot.main import (
    initialize_heartbeat_job,
...
async def test_run_main_creates_scheduler_task_with_configured_tick(
    ...
    mock_init_cleanup.assert_awaited_once()
...
@patch("src.sohnbot.main.create_job", new_callable=AsyncMock)
@patch("src.sohnbot.main.get_job_by_name", new_callable=AsyncMock)
async def test_initialize_operation_logs_cleanup_job_creates_when_missing(
    mock_get_job_by_name,
    mock_create_job,
):
    mock_get_job_by_name.return_value = None
    mock_create_job.return_value = {"id": "job-cleanup"}

    await initialize_operation_logs_cleanup_job()

    mock_create_job.assert_awaited_once()
    kwargs = mock_create_job.call_args.kwargs
    assert kwargs["name"] == "Operation Logs Cleanup"
    assert kwargs["cron_expr"] == "0 3 * * 0"
    assert kwargs["timezone"] == "UTC"
    assert kwargs["action"] == "cleanup_operation_logs"
    assert kwargs["action_params"] == {"retention_days": 90}


@pytest.mark.asyncio
@patch("src.sohnbot.main.create_job", new_callable=AsyncMock)
@patch("src.sohnbot.main.get_job_by_name", new_callable=AsyncMock)
async def test_initialize_operation_logs_cleanup_job_idempotent(
    mock_get_job_by_name,
    mock_create_job,
):
    mock_get_job_by_name.return_value = {"id": "existing-cleanup"}
